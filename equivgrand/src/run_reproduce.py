...
def train(model, optimizer, data, pos_encoding=None):
  ...
  if model.opt['use_labels']:
    ...
    train_label_idx, _ = get_label_masks(data, model.opt['label_rate'])
...
def equiv_train(model, optimizer, data, pos_encoding=None):
  model.train()
  optimizer.zero_grad()
  feat = data.x
  coord = feat[:, :3].clone()
  
  out = model(feat, coord, pos_encoding)
...
@torch.no_grad()
def equiv_test(model, data, pos_encoding=None, opt=None):  # opt required for runtime polymorphism
  model.eval()
  feat = data.x
  coord = feat[:, :3].clone()
    
  logits, accs = model(feat, coord, pos_encoding), []
  for _, mask in data('train_mask', 'val_mask', 'test_mask'):
    pred = logits[mask].max(1)[1]
    acc = pred.eq(data.y[mask]).sum().item() / mask.sum().item()
    accs.append(acc)
...
def merge_cmd_args(cmd_opt, opt):
  ...
  if cmd_opt['attention_type'] is not None:
  ...
  if cmd_opt['rewiring'] is not None:
    opt['rewiring'] = cmd_opt['rewiring']    
  if cmd_opt['max_nfe'] != 1000:
...
def main(opt): 
  ...
  for split in tqdm(range(opt['num_splits'])):
    print('*'*50)
    print('Data split #', split + 1)
    ...
    # run 20 randomly initialized model on each split
    for init in range(20):
      print()
      print('Random init #', init + 1, flush=True)
      if opt['equiv']:
        model = EquivGNN(opt, dataset, device).to(device)
      elif opt['rewire_KNN'] or opt['fa_layer']:
      ...
      if opt['equiv']:
        this_train, this_test = equiv_train, equiv_test
      else:
      ...
      for epoch in range(1, opt['epoch']):
        ...
        torch.cuda.empty_cache() 
        # if epoch % 20 == 0:
        #   print(log.format(epoch, time.time() - start_time, loss, model.fm.sum, model.bm.sum, train_acc, val_acc, test_acc, best_time), flush=True)
        
      print('Best val accuracy {:03f} with test accuracy {:03f} at epoch {:d} and best time {:03f}'.format(val_acc, test_acc, best_epoch, best_time), flush=True)
      test_results[split].append(test_acc)
      test_accs.append(test_acc)
   
  opt['test_results'] = test_results 
  opt['test_acc_mean'] = np.array(test_accs).mean().item()
  opt['test_acc_std'] = np.array(test_accs).std().item()
...
if __name__ == '__main__':
  ...
  # parser.add_argument('--self_loop_weight', type=float, default=1, help='Weight of self-loops.')
  ...
  # GNN args
  parser.add_argument("--equiv", action="store_true", help="whether to use EquivariantGRAND or not") 
  ...
  parser.add_argument('--block', type=str, default=None, help='constant, mixed, attention, hard_attention')
  parser.add_argument('--function', type=str, default=None, help='laplacian, transformer, dorsey, GAT')
  ...
  parser.add_argument('--method', type=str, default=None, help="set the numerical solver: dopri5, euler, rk4, midpoint")
