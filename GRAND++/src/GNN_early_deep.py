...
class GNNEarlyDeep(BaseGNN):
  def __init__(self, opt, dataset, device=torch.device('cpu')):
    super(GNNEarlyDeep, self).__init__(opt, dataset, device)
    ...
    self.odeblock = []
    for i in range(opt['num_ode_block']):
      self.odeblock.append(block(self.f, self.regularization_fns, opt, dataset.data, device, t=time_tensor).to(device))
      # overwrite the test integrator with this custom one
      self.odeblock[-1].test_integrator = EarlyStopInt(self.T, opt, device)
    # if opt['adjoint']:
    ...
    self.num_ode_block = opt['num_ode_block']
    
    self.set_solver_data(dataset.data)
    
    self.reg_states = []

  def set_solver_m2(self, indx): # TODO: Check this 
    if self.odeblock[indx].test_integrator.m2 is None:
      self.odeblock[indx].test_integrator.m2 = self.m2
    else:
      self.odeblock[indx].test_integrator.m2.weight.data = self.m2.weight.data
      self.odeblock[indx].test_integrator.m2.bias.data = self.m2.bias.data

  def set_solver_data(self, data): # TODO: Check this
    for i in range(self.num_ode_block):
      self.odeblock[i].test_integrator.data = data 
  ...
  def forward(self, x):
    ...
    x = F.dropout(x, self.opt['input_dropout'], training=self.training) # x: 2485 x 1433
    x = self.m1(x) # x: 2485 x 80
    ...
    for i in range(self.num_ode_block):
      ...
      self.odeblock[i].set_x0(x) # x: 2485 x 80
      self.set_solver_m2(i)

      if self.training  and self.odeblock[i].nreg > 0:
        x, reg_states_val  = self.odeblock[i](x)
        self.reg_states.append(reg_states_val)
      else:
        x = self.odeblock[i](x) # x: 2485 x 80, z: 2485 x 80

      if self.opt['augment']:
        x = torch.split(x, x.shape[1] // 2, dim=1)[0]

      # Activation.
      x = F.relu(x) # z: 2485 x 80
      ...
      # Dropout.
      x = F.dropout(x, self.opt['dropout'], training=self.training) # z: 2485 x 80

    # Decode each node embedding to get node label.
    z = self.m2(x) # z: 2485 x 7
    return z
