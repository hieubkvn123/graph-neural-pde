...
from GNN import GNN
from GNN_early_deep import GNNEarlyDeep as GNNEarly
import time
from data_default import get_dataset, set_train_val_test_split
...
def main(cmd_opt):
  ...
  if not opt['planetoid_split'] and opt['dataset'] in ['Cora', 'Citeseer', 'Pubmed']:
    dataset.data = set_train_val_test_split(opt['seed'], dataset.data,
                                            num_development=5000 if opt["dataset"] == "CoauthorCS" else 1500) # Tan TODO
  ...
  for epoch in range(1, opt['epoch']):
    ...
    if tmp_val_acc > val_acc:
      ...
      best_time = opt['time']
    if not opt['no_early'] and model.odeblock.test_integrator.solver.best_val > val_acc: #TODO: Check this
      best_epoch = epoch
...
if __name__ == '__main__':
  ...
  parser.add_argument('--num_ode_block', type=int, default=1, help='number of ode blocks')
  ...
  with open("/tanData/graph_poisson_network/results/all_results_new.txt", "a") as f:
    ...
    f.write('mean results: mean best val accuracy {:03f} with mean test accuracy {:03f} and mean train accuracy {:03f} after {} runs'.format(val_acc, test_acc, train_acc, opt['num_runs']))
    fcntl.flock(f, fcntl.LOCK_UN)
