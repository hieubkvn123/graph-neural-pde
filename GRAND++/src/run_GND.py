...
def main(cmd_opt):
  ...
  model = ICML_GNN(opt, dataset, opt['time'], device).to(device) if opt["no_early"] else GNNEarly(opt, dataset, device).to(device)
  print(opt)
  ...
  print('best val accuracy {:03f} with test accuracy {:03f} at epoch {:d} and best time {:03f}'.format(val_acc, test_acc,
                                                                                                     best_epoch,
                                                                                                     best_time))
  with open('results_GND_%s.txt'%opt['dataset'], 'w') as f:
    sys.stdout = f
    print('Seed {:d}: best val accuracy {:03f} with test accuracy {:03f} at epoch {:d} and best time {:03f}'.format(opt['manualSeed'],val_acc, test_acc, best_epoch, best_time))
