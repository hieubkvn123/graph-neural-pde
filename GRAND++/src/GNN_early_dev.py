...
class GNNEarly(BaseGNN):
  ...
  def forward(self, x):
    ...
    self.odeblock.set_x0(x) # x: 2485 x 80
    self.set_solver_m2()

    if self.training  and self.odeblock.nreg > 0:
      z, self.reg_states  = self.odeblock(x)
    else:
      z = self.odeblock(x) # x: 2485 x 80, z: 2485 x 80
      
    if self.opt['augment']:
      z = torch.split(z, x.shape[1] // 2, dim=1)[0]

    # Activation.
    z = F.relu(z) # z: 2485 x 80
    ...
    # Dropout.
    z = F.dropout(z, self.opt['dropout'], training=self.training) # z: 2485 x 80

    # Decode each node embedding to get node label.
    z = self.m2(z) # z: 2485 x 7
    return z
